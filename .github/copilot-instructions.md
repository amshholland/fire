...
- **Port**: 3030 by default (configurable via `APP_PORT` in env)
...
```bash
npm start              # Concurrently start backend (3030) + frontend (3000)
...
- `APP_PORT=3030` - Server port (optional)
...
- Backend default is `:3030` (see `backend/src/config/env.ts`)
